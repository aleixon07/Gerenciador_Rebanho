...
if (isset($_GET['id'])) {
    ...
    if (!empty($del_id)) {
        ...
        $sql_animal = "SELECT * FROM animal WHERE idCategoria = '$del_id'";
        ...
        if(mysqli_num_rows($result_animal) > 0){
            $sql_delete_animal = "DELETE FROM animal WHERE idCategoria ='$del_id'";
        ...
        $sql = "DELETE FROM categoria WHERE idCategoria ='$del_id'";
        try {
            $result = mysqli_query($conn, $sql);
        } catch (Exception $e) {
            header("Location: ../dashboard/sidebar_categoria.php?e=a87ff679a2f3e71d9181a67b7542122c");
            exit();
        }
        if ($result) {
            header("Location: ../dashboard/sidebar_categoria.php?e=c81e728d9d4c2f636f067f89cc14862c");
            exit();
        }
    }
} else {
